...
$.fn.calculatevoip = function(){ 
        ...
        var unlimited_total = ((5 * unlimited) + (slider_phones * paygphones));
        var clean_unlimited_total = addCommas(Math.round(unlimited_total));
        
        var metered_total = ((5 * metered) + (slider_phones * paygphones) + (slider_outmin * metered_min_cost) + (slider_minutes * metered_min_cost));
        var clean_metered_total = addCommas(Math.round(metered_total));
        
        } else {
        
        var unlimited_total = ((slider_phones * unlimited) + (slider_phones * paygphones));
        var clean_unlimited_total = addCommas(Math.round(unlimited_total));
        
        var metered_total = ((slider_phones * metered) + (slider_phones * paygphones) + (slider_outmin * metered_min_cost) + (slider_minutes * metered_min_cost));
        var clean_metered_total = addCommas(Math.round(metered_total));
        ...
        var paygtotal = ((slider_phones * paygphones) + (slider_minutes * metered_min_cost) + (slider_outmin * metered_min_cost) + paygbase);
        var clean_paygtotal_total = addCommas(Math.round(paygtotal));
        
        } else {
        
        var paygtotal = ((slider_phones * paygphones) + (slider_minutes * metered_min_cost) + (slider_outmin * metered_min_cost) + ((slider_vmboxes - 5) * paygphones ) + paygbase);
        var clean_paygtotal_total = addCommas(Math.round(paygtotal));
        ...
        if(metered_total > user_cost) {
        
         $('#meteredsavings').html("No cost savings.");
         
        } else {
        
        $('#meteredsavings').html(Math.round(meteredsavings) + '% Cost Savings');
...
var voip2 = $("#unranged-value-permonth").freshslider({
        ...
        var usertotal ="$1,500 - $5,000 /mo";
        $(function() {
	  $.fn.calculatevoip();
	});
        }
        if((value < 10001)&&(value > 5000)) {
        var usertotal ="$5,000 - $10,00 /mo +";
        $(function() {
	  $.fn.calculatevoip();
	});
        }
